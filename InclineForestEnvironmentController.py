...
#break
    speedav += (E100_functions.get_linear_velocity(client)[0]**2+E100_functions.get_linear_velocity(client)[1]**2)**0.5
    speedcount += 1
    #################### get sensor readings #####################      
    ...
    if (step == 0): #go forward
        E100_functions.set_quadcopter(client,0,-0.9*speed,0,0.85*throttle)
        if((front<12.5) and (front>0) ):
            step = 1 
        
    
    if (step == 1): #go slow down going forward
        E100_functions.set_quadcopter(client,0,5.2*speed,0,1*throttle)
    ...
    if (step == 2): #go right
        ...
        if ((right<30) or (front>25) or( math.isnan(front)==1)):
            step = 3
    if (step == 20): #go left
        E100_functions.set_quadcopter(client,-1*speed,0,0,throttle)
        if ((left<30) or (front>25) or  (math.isnan(front)==1)):
            step = 30
